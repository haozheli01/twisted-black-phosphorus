...
class TwistedBPModel:
    def __init__(self, N_shell=1, E_field=0.0, 
        ...
        self.delta_g_val = 2 * np.pi * np.abs(1/b_lat - 1/a_lat) # Moiré G vector magnitude for real lattice
        self.gamma_ML, self.gamma_C, self.gamma_V = gamma_ML, gamma_C, gamma_V # coupling parameters in eV
        ...
        self.beta_ML = beta_ML # additional coupling parameter in eV
    ...
    def _build_constant_hamiltonian(self):
        ...
        H[cols * 4 + 3, rows * 4 + 1] = self.gamma_V # Hermiticity
...
def calculate_shift_current(N_shell=1, E_field=0.0,
                            ...
                            k_range=0.15, n_k=60, comp=('x', 'x', 'x'),
                            band_window=None, chunk_size=200000, save_prefix=""):
    """
    Calculate shift current spectrum sigma^{abc}(ω) (a.u.).

    Parameters
    ----------
    comp : tuple(str,str,str)
        Tensor component (a,b,c), each in {'x','y'}.
        a: shift vector direction, b/c: optical polarizations.
    band_window : tuple or None
        (v_min, v_max, c_min, c_max) band index ranges to restrict transitions.
        If None, use valence < mid, conduction >= mid.
    chunk_size : int
        Number of transitions per chunk for spectral accumulation.
    """

    print(f"Calculating shift current for component \sigma^{comp[0]}{comp[1]}{comp[2]}(\omega)...")
    model = TwistedBPModel(N_shell=N_shell, E_field=E_field)
    ...
    # --- diagonalize ---
    H_stack = model.get_hamiltonians(k_points)
    evals, evecs = np.linalg.eigh(H_stack)  # evals: (Nk, Nb), evecs: (Nk, Nb, Nb)
    ...
    if band_window is None:
        ...
        v_min, v_max, c_min, c_max = band_window
        v_idx = np.arange(v_min, v_max+1)  # 0-based
        c_idx = np.arange(c_min, c_max+1)  # 0-based

    # --- energies for transitions ---
    E_v = evals[:, v_idx]  # (Nk, Nv)
    E_c = evals[:, c_idx]  # (Nk, Nc)
    delta_E = E_c[:, None, :] - E_v[:, :, None]  # (Nk, Nv, Nc)

    # --- r^b_{cv} and r^c_{cv} ---
    v_b_vc = v_b[:, v_idx[:, None], c_idx[None, :]]  # (Nk, Nv, Nc)
    v_c_vc = v_c[:, v_idx[:, None], c_idx[None, :]]  # (Nk, Nv, Nc)
    eps = 1e-9
    r_b = v_b_vc / (1j * (delta_E + 1j * eps))
    r_c = v_c_vc / (1j * (delta_E + 1j * eps))

    # --- reshape to grid for derivatives ---
    Nv = len(v_idx)
    Nc = len(c_idx)
    r_b_grid = r_b.reshape(n_k, n_k, Nv, Nc)

    # --- Berry connection A^a_nn from link variables ---
    # reshape eigenvectors: (n_k, n_k, Nb, Nb)
    U = evecs.reshape(n_k, n_k, Nb, Nb)

    if a_dir == 'x':
        # overlap along +x
        overlap = np.einsum('ijna,ijna->ijn', np.conj(U[:-1, :, :, :]), U[1:, :, :, :])
        A_a = np.zeros((n_k, n_k, Nb), dtype=np.float64)
        A_a[:-1, :, :] = np.imag(np.log(overlap)) / dkx
        A_a[-1, :, :] = A_a[-2, :, :]  # edge
        # phase derivative along x
        phi = np.unwrap(np.angle(r_b_grid), axis=0)
        dphi = (np.roll(phi, -1, axis=0) - np.roll(phi, 1, axis=0)) / (2 * dkx)
    else:
        # overlap along +y
        overlap = np.einsum('ijna,ijna->ijn', np.conj(U[:, :-1, :, :]), U[:, 1:, :, :])
        A_a = np.zeros((n_k, n_k, Nb), dtype=np.float64)
        A_a[:, :-1, :] = np.imag(np.log(overlap)) / dky
        A_a[:, -1, :] = A_a[:, -2, :]
        # phase derivative along y
        phi = np.unwrap(np.angle(r_b_grid), axis=1)
        dphi = (np.roll(phi, -1, axis=1) - np.roll(phi, 1, axis=1)) / (2 * dky)

    # --- shift vector R^a_{cv} ---
    A_c = A_a[:, :, c_idx]  # (n_k, n_k, Nc)
    A_v = A_a[:, :, v_idx]  # (n_k, n_k, Nv)
    A_diff = A_c[:, :, None, :] - A_v[:, :, :, None]  # (n_k, n_k, Nv, Nc)

    R_a = dphi - A_diff  # (n_k, n_k, Nv, Nc)

    # --- integrand weight: Re[r^b_{cv} r^c_{vc}] * R^a_{cv} ---
    r_b_grid = r_b_grid
    r_c_grid = r_c.reshape(n_k, n_k, Nv, Nc)
    weight = np.real(r_b_grid * np.conj(r_c_grid) * R_a)  # (n_k, n_k, Nv, Nc)

    # --- flatten transitions ---
    delta_E_flat = delta_E.reshape(-1)
    ...
    print(f"  Summing transitions (chunksize={chunk_size})...")
    n_trans = len(delta_E_flat)
    for s in range(0, n_trans, chunk_size):
        e_chunk = delta_E_flat[s:s + chunk_size]
        w_chunk = weight_flat[s:s + chunk_size]
        diff = omegas[:, None] - e_chunk[None, :]
        lorentz = (1 / np.pi) * eta / (diff**2 + eta**2)
        sigma += np.sum(lorentz * w_chunk[None, :], axis=1)

    sigma /= Nk

    # --- plot ---
    plt.figure(figsize=(8, 6))
    plt.plot(omegas, sigma, 'm-', lw=2, label=fr'$\sigma^{{{a_dir}{b_dir}{c_dir}}}(\omega)$')
    plt.xlabel('Photon Energy (eV)')
    plt.ylabel('Shift Current (a.u.)')
    plt.title(f'E={E_field} eV/A, η={eta*1000:.1f} meV')
    plt.grid(True, alpha=0.3)
    plt.xlim(E_range)
    plt.legend()

    suffix = f"_Shift_{a_dir}{b_dir}{c_dir}_E{E_field:.3f}{save_prefix}.png"
    plt.savefig(f"TB{suffix}", dpi=300)
    plt.close()
    print(f"Saved Shift Current Spectrum: TB{suffix}")
...
if __name__ == "__main__":
    ...
    # --------------------------------------------
    calculate_shift_current(N_shell=1, E_field=0.0, n_k=200, n_E=400, comp=('x', 'y', 'y'),
                            band_window=(8,9,10,10), chunk_size=200000,k_range=G_moire/2,)