...
def calculate_shift_current(N_shell=1, E_field=0.0,
    ...
    Formula: sigma^{abc} ~ sum_nm f_nm * Im[ r^b_nm * (r^c_mn)_{;a} ] * delta(w - w_mn)
    """
    ...
    wxx_orb, wyy_orb, wxy_orb = model.get_generalized_derivative_matrices(k_points)
    ...
    w_map = {'xx': to_eig(wxx_orb), 'yy': to_eig(wyy_orb), 
             'xy': to_eig(wxy_orb), 'yx': to_eig(wxy_orb)} # wxy symmetric here

    v_a = v_map[a_dir]
    v_b = v_map[b_dir]
    v_c = v_map[c_dir]
    
    # For second derivative w_{ac}
    key_w = a_dir + c_dir if (a_dir + c_dir) in w_map else c_dir + a_dir
    w_ac = w_map[key_w]
    ...
    for n in v_idx:
        for m in c_idx:
            ...
            nonzero_mask = w_mn > 1e-4
            
            # --- Dipole Matrix Elements ---
            # r_nm = v_nm / (i * w_mn)
            r_b_nm = np.zeros(Nk, dtype=np.complex128)
            r_b_nm[nonzero_mask] = v_b[nonzero_mask, n, m] / (1j * w_mn[nonzero_mask])
            
            # --- Generalized Derivative (r^c_mn)_{;a} ---
            # Formula: (r^c_mn)_{;a} = (1/w_mn) * [ i*w^ac_mn + sum_{p!=n,m} (...) ]
            
            # Sum over intermediate states p
            sum_sos = np.zeros(Nk, dtype=np.complex128)
            
            for p in range(Nb):
                if p == n or p == m: continue
                
                w_pn = evals[:, p] - evals[:, n]
                w_mp = evals[:, m] - evals[:, p]
                
                # Check gaps
                p_mask = nonzero_mask & (np.abs(w_pn) > 1e-4) & (np.abs(w_mp) > 1e-4)
                
                # SOS Terms: v^a_mp * r^c_pn - r^c_mp * v^a_pn
                # r^c_pn = v^c_pn / (i * w_pn)
                # r^c_mp = v^c_mp / (i * w_mp)
                
                if np.any(p_mask):
                    term1 = v_a[p_mask, m, p] * (v_c[p_mask, p, n] / (1j * w_pn[p_mask]))
                    term2 = (v_c[p_mask, m, p] / (1j * w_mp[p_mask])) * v_a[p_mask, p, n]
                    sum_sos[p_mask] += (term1 - term2)
            
            # Construct generalized derivative
            # K_mn = i * w^ac_mn + sum_sos
            K_mn = 1j * w_ac[:, m, n] + sum_sos
            
            r_c_mn_semicolon_a = np.zeros(Nk, dtype=np.complex128)
            r_c_mn_semicolon_a[nonzero_mask] = K_mn[nonzero_mask] / w_mn[nonzero_mask]
            
            # --- Shift Current Contribution ---
            # Weight ~ Im[ r^b_nm * (r^c_mn)_{;a} ]
            weight = np.imag(r_b_nm * r_c_mn_semicolon_a)
    ...
    plt.xlabel('Photon Energy (eV)')
    plt.ylabel('Shift Conductivity (arb. units)')
    plt.title(f'Shift Current Spectrum (SOS Method)\nE={E_field} eV/A')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.xlim(E_range)
    
    suffix = f"_ShiftSOS_{a_dir}{b_dir}{c_dir}_E{E_field:.3f}{save_prefix}.png"
    plt.savefig(f"TB{suffix}", dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    calculate_shift_current(N_shell=1, E_field=0.0, n_k=200, n_E=400, comp=('x', 'x', 'y'),
                            band_window=(8,9,10,10), save_prefix="_test", k_range=G_moire/2,)