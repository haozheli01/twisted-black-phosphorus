...
class TwistedBPModel:
    def __init__(self, 
        ...
        self.coupling = 0.22 # For 2+2/3+3, 0.095 is a good fit . For 4+4, 0.07 is a good fit.
    ...
    def get_hamiltonians(self, k_points):
        ...
        # Inter-block coupling (2x2) between top and bottom blocks
        # H_full[:, :self.N_top*2, self.N_top*2:] = self.coupling * np.ones((self.N_top * 2, self.N_bottom * 2))
        # H_full[:, self.N_top*2:, :self.N_top*2] = self.coupling * np.ones((self.N_bottom * 2, self.N_top * 2))

        H_full[:, (self.N_top-1)*2:self.N_top*2, self.N_top*2:(self.N_top+1)*2] = self.coupling * np.ones((2, 2))
        H_full[:, self.N_top*2:(self.N_top+1)*2, (self.N_top-1)*2:self.N_top*2] = self.coupling * np.ones((2, 2))


        # # Treatment of Nature Electronics volume 4, pages269–276 (2021)
        # H_full[:, (self.N_top-1)*2:self.N_top*2, self.N_top*2:(self.N_top+1)*2] = (Hc_top + Hc_bot) * 0.05
        # H_full[:, self.N_top*2:(self.N_top+1)*2, (self.N_top-1)*2:self.N_top*2] = np.conj(Hc_top + Hc_bot).transpose((0,2,1)) * 0.05
...
if __name__ == "__main__":
    ...
    cal_bands(N_top=n_top, N_bottom=n_bottom, twist_angle=twist_angle,
              k_fine_steps=200, y_lim=(-4.0,4.0))
